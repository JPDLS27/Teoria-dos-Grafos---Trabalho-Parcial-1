...
public class Grafo {
        ...
        for (int i = 0; i < numeroVertices; i++) {
            ...
            for (int j = 0; j < numeroVertices; j++) {
                if (ehDirigido) {
                    ...
                    if (i == j) grau += matrizAdjacencia[i][j]; // laços contam dobrado
                }
            }
            graus.add(grau);
        }
     ...
     * @return String identificando o grau do grafo, de cada vértice e a sequência de graus
     */
    public String grausDoVertice() {
        ...
        if (grauMinimo == Integer.MAX_VALUE) grauMinimo = 0;
